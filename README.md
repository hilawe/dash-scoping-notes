...
- [d2-batch-rotation-spec.md](d2-batch-rotation-spec.md) ([PDF](d2-batch-rotation-spec.pdf)), the implementation spec for D2, a wallet privacy policy that spends from a small transparent pot refilled from a shielded reserve in sealed, never-co-spent batches. Covers the state machine, default parameters, the six residual risks, and the measurement gate.
- [d4-coinbase-maturity.md](d4-coinbase-maturity.md) ([PDF](d4-coinbase-maturity.pdf)), the writeup for D4, relaxing the 100-block coinbase maturity rule now that ChainLocks make deep reorgs practically impossible. Covers the one real design problem (a syncing node cannot locally prove a historical block was ChainLocked), three options, and why it should ride the next hard fork rather than run its own.
